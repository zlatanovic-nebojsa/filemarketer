	...
	@slot('links')
		<div class="level">
			<div class="level-left">
				...
				<form action="{{ route('admin.files.updated.update', $file) }}" id="approve-{{ $file->id }}" class="is-hidden" method="post">
					{{ csrf_field() }}
					{{ method_field('PATCH')}}
				</form>
				...
				<form action="{{ route('admin.files.updated.destroy', $file) }}" id="reject-{{ $file->id }}" class="is-hidden" method="post">
					{{ csrf_field() }}
					{{ method_field('DELETE')}}
				</form>
			</div>
